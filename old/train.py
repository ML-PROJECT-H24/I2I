...
model = models.MDTv2_S_2().to(device)
...
if not args.gen_only:
  ...
  for step, (mean, logvar) in enumerate(dataloader):
      ...
      y_null = torch.tensor([0], device=device)
      model_kwargs = dict(y=y_null)
      #TODO enable masking

      losses = diff.training_losses(model, x_0, t, model_kwargs=model_kwargs)
...
for i in range(10):
    ...
    model_kwargs = dict(y=y_null, cfg_scale=cfg_scale, scale_pow=pow_scale) 

    # Generate images

    x = diff.p_sample_loop(model.forward_with_cfg, z.shape, z, clip_denoised=False, progress=True, device=device, model_kwargs=model_kwargs)
    ...
    name = f"generated_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_{i}.png"
